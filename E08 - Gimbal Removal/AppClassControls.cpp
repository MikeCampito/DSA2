...
{
	/*glm::quat locQuat = glm::quat();
	m_v3Rotation = vector3(0.0f, 0.0f, 0.0f);
	locQuat.w = cosf(0.0174533f / 2);
	if (a_event.key.code == sf::Keyboard::X) {
		locQuat.x = 1.0f * sinf(0.0174533f / 2);
	}
	else {
		locQuat.x = 0.0f;
	}
	if (a_event.key.code == sf::Keyboard::Y) {
		locQuat.y = 1.0f * sinf(0.0174533f / 2);
	}
	else {
		locQuat.y = 0.0f;
	}
	if (a_event.key.code == sf::Keyboard::Z) {
		locQuat.z = 1.0f * sinf(0.0174533f / 2);
	}
	else {
		locQuat.z = 0.0f;
	}*/
...
void Application::ProcessKeyboard(void)
{
	...
	m_qOrientation = locQuat * m_qOrientation;
	/*
