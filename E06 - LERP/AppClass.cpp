...
void Application::InitVariables(void)
{
	...
	m_sProgrammer = "Mike Campito - mjc1264@g.rit.edu";
...
void Application::Display(void)
{
	...
	//your code goes here
	if (index == m_stopsList.size()-1) {
		v3CurrentPos = (1 - fTimer)*m_stopsList[index] + fTimer * m_stopsList[0];
		if (fTimer>=1) {
			index = 0;
			fTimer = 0;
		}
		...
		v3CurrentPos = (1 - fTimer)*m_stopsList[index] + fTimer * m_stopsList[index+1];
		if (fTimer >= 1) {
			index++;
			fTimer = 0;
		}
